...
v1_labels = np.round(v1_ticks/32)
...
air_Ez_list = air_Ez[:,64,:]
k_val =np.pi/a1 
# number of periods
translation = np.arange(1,4,1)
# Apply the translations to Ez and stack them vertically
for t in translation:
    air_translated_Ez = air_Ez[:,64,:]*np.exp(1j*k_val*t*a1)
    air_Ez_list= np.vstack((air_Ez_list,air_translated_Ez))
plt.imshow(np.real(air_Ez_list), cmap ='RdBu')
h1_ticks = np.linspace(1, 128, 4)
h1_labels = np.round(h1_ticks/32)
plt.xticks(h1_ticks, h1_labels)
v1_ticks =np.linspace(0,len(Ez_list), 2*int(len(Ez_list)/len(Ez)))
v1_labels = np.round(v1_ticks/32)
plt.yticks(v1_ticks, v1_labels)
plt.xlabel('Z')
plt.ylabel('X unit cell')
plt.colorbar()
...
# mlab.contour3d(eps_fu, colormap = 'hot')
...
# mlab.pipeline.grid_plane(mlab.pipeline.scalar_field(eps_fu), plane_orientation='x_axes', color=(0.5, 0.5, 0.5))
# mlab.pipeline.grid_plane(mlab.pipeline.scalar_field(eps_fu), plane_orientation='y_axes', color=(0.5, 0.5, 0.5))
# mlab.show()
